...
# your block variables in such a way that it ignores the first two elements, and groups
# all remaining elements as a raptors array.

def types(birds_array)
  yield(birds_array)
end

p (types(%w(raven finch hawk eagle)) do |_, _, *raptors|
  "Raptor array: #{raptors}" 
end)
