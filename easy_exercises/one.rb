...
# If the block returns a value of true for exactly one element,
# then #one? returns true. Otherwise, #one? returns false. Note
# in particular that #one? will stop searching the collection the
...
def one?(array)
  ...
  array.each do |element|
    true_counter += 1 if yield(element)
    break false if true_counter > 1
  end
  true_counter == 1
end
