...
# each value in the iteration is the previous value plus a "step"
# value. It returns the original range.
...
# to each iteration. Your method should also take a block to which it
# will yield (or call) successive iteration values.

# Example:
# step(1, 10, 3) { |value| puts "value = #{value}" }

def step(starter, ender, stepper)
  step = starter
  until step > ender do
    yield(step)
    step += stepper
