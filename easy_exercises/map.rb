...
# up into an Array that is then returned to the caller. Note in particular
# that every time #map yields to the block, it obtains just one value.
# That value may be a complex value - it may even be another collection -
# but it is nevertheless just one value. Thus, #map returns an Array
# that has the same number of elements that the original collection had.
...
# that contains the return values produced by the block for each element of
# the original Array.
